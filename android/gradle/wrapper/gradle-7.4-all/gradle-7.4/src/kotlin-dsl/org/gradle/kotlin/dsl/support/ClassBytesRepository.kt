...
@VisibleForTesting
fun classPathBytesRepositoryFor(
    classPath: List<File>,
...
class ClassBytesRepository(
    classPath: ClassPath,
    classPathDependencies: ClassPath = ClassPath.EMPTY
) : Closeable {
    ...
    private
    val classBytesIndex =
        (classPathFiles + classPathDependencies.asFiles).map { classBytesIndexFor(it) }
    ...
    fun directoryClassBytesIndexFor(dir: File): ClassBytesIndex = { classFilePath ->
        dir.resolve(classFilePath).takeIf { it.isFile }
            ?.let { classFile -> { classFile.readBytes() } }
    }
