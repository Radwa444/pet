...
class PluginAccessorClassPathGenerator @Inject constructor(
    ...
    fun pluginSpecBuildersClassPath(project: ProjectInternal): AccessorsClassPath =
        project.owner.owner.projects.rootProject.mutableModel.let { rootProject ->
            ...
            rootProject.getOrCreateProperty("gradleKotlinDsl.pluginAccessorsClassPath") {
                ...
                val classLoaderHash = requireNotNull(
                    classLoaderHierarchyHasher.getClassLoaderHash(buildSrcClassLoaderScope.exportClassLoader)
                )
...
class GeneratePluginAccessors(
        ...
        kotlinScriptClassPathProviderOf(rootProject).run {
            withAsynchronousIO(rootProject) {
                buildPluginAccessorsFor(
                    pluginDescriptorsClassPath = exportClassPathFromHierarchyOf(
                        buildSrcClassLoaderScope
                    ),
    ...
    ) = UnitOfWork.Identity { classLoaderHash.toString() }
    ...
    override fun getDisplayName(): String =
        "Kotlin DSL plugin accessors for classpath '$classLoaderHash'"
    ...
    override fun visitOutputs(workspace: File, visitor: UnitOfWork.OutputVisitor) {
        ...
        visitor.visitOutputProperty(
            SOURCES_OUTPUT_PROPERTY,
            DIRECTORY,
            sourcesOutputDir,
            fileCollectionFactory.fixed(sourcesOutputDir)
        )
        visitor.visitOutputProperty(
            CLASSES_OUTPUT_PROPERTY,
            DIRECTORY,
            classesOutputDir,
            fileCollectionFactory.fixed(classesOutputDir)
        )
    }
}
    ...
    val header = writeFileFacadeClassHeader(moduleName) {
        accessorList.forEach { accessor ->
            ...
            val getterSignature = jvmGetterSignatureFor(
                propertyName,
                "(L${receiverType.internalName};)L${returnType.internalName};"
            )
            writePropertyOf(
    ...
    publicStaticMethod(signature) {
        when (accessor) {
            ...
            is PluginAccessor.ForPlugin -> {
                ...
                LDC(accessor.id)
                INVOKEINTERFACE(
                    pluginDependenciesSpecInternalName,
                    "id",
                    pluginDependenciesSpecIdMethodDesc
                )
                ARETURN()
            }
        }
            ...
            is PluginAccessor.ForPlugin -> {
                appendReproducibleNewLine(
                    format(
                        """
                        /**
                         * The `$id` plugin implemented by [${
                            sourceNameOfBinaryName(
                                implementationClass
                            )
                        }].
                         */
...
internal
val pluginDependenciesSpecTypeSpec =
    TypeSpec("PluginDependenciesSpec", pluginDependenciesSpecInternalName)


internal
val pluginDependencySpecTypeSpec =
    TypeSpec("PluginDependencySpec", pluginDependencySpecInternalName)
