    ...
    inner class InterpreterHost : Interpreter.Host {
        ...
        override fun pluginAccessorsFor(scriptHost: KotlinScriptHost<*>): ClassPath =
            (scriptHost.target as? ProjectInternal)?.let {
                val pluginAccessorClassPathGenerator =
                    it.serviceOf<PluginAccessorClassPathGenerator>()
                pluginAccessorClassPathGenerator.pluginSpecBuildersClassPath(it).bin
            } ?: ClassPath.EMPTY

        override fun runCompileBuildOperation(
            scriptPath: String,
            stage: String,
            action: () -> String
        ): String =
            ...
            buildOperationExecutor.call(object : CallableBuildOperation<String> {
                ...
                override fun description(): BuildOperationDescriptor.Builder {
                    val name =
                        "Compile script ${scriptPath.substringAfterLast(File.separator)} ($stage)"
                    return BuildOperationDescriptor.displayName(name).name(name)
        ...
        override fun loadClassInChildScopeOf(
            ...
            val instrumentedClasses =
                cachedClasspathTransformer.transform(DefaultClassPath.of(location), BuildLogic)
