...
class KotlinApiMemberWriter(apiMemberAdapter: ClassVisitor) : ApiMemberWriter(apiMemberAdapter) {
    ...
    override fun writeClass(
        ...
        methods: Set<MethodMember>,
        fields: Set<FieldMember>,
        innerClasses: Set<InnerClassMember>
    ) {
        classMember.annotations.firstOrNull {
            ...
            when (val kotlinMetadata = KotlinClassMetadata.read(parseKotlinClassHeader(it))) {
                ...
                is KotlinClassMetadata.FileFacade -> kotlinMetadata.toKmPackage()
                    .extractFunctionMetadata()

                is KotlinClassMetadata.MultiFileClassPart -> kotlinMetadata.toKmPackage()
                    .extractFunctionMetadata()
    ...
    override fun writeClassAnnotations(annotationMembers: Set<AnnotationMember>) {
        super.writeClassAnnotations(annotationMembers.filter { it.name != kotlinMetadataAnnotationSignature }
            .toSet())
    }
        ...
        kotlinMetadataAnnotation.values.forEach {
            ...
            when (it) {
                ...
                is ArrayAnnotationValue ->
                    when (it.name) {
                        "d1" -> data1 =
                            it.value.map { arrayItem -> arrayItem.value as String }.toTypedArray()
...
class CompileAvoidanceException(message: String) : GradleException(message) {
    ...
    companion object Factory {
        fun publicInlineFunction(inlineFunction: MethodMember) =
            CompileAvoidanceException("inline fun ${inlineFunction.name}(): compile avoidance is not supported with public inline functions")

        fun withClass(className: String, e: CompileAvoidanceException) =
            CompileAvoidanceException("class $className: ${e.message}")
    }
}
