...
fun apiTypeProviderFor(
    ...
    ApiTypeProvider(
        classPathBytesRepositoryFor(classPath, classPathDependencies),
...
fun ApiTypeProvider.Context.apiFunctionParametersFor(
    ...
    delegate.visibleParameterAnnotations?.map { it.has<Nullable>() }.let { parametersNullability ->
        ...
        parameterTypesBinaryNames.mapIndexed { idx, parameterTypeBinaryName ->
            ...
            val typeArguments =
                signatureParameter?.typeArguments ?: emptyList<TypeSignatureVisitor>()
...
private
fun ApiTypeProvider.Context.apiTypeUsageForReturnType(
    delegate: MethodNode,
    returnType: TypeSignatureVisitor?
) =
    apiTypeUsageFor(
...
private
class TypeSignatureVisitor(val variance: Variance = Variance.INVARIANT) :
    SignatureVisitor(ASM_LEVEL) {
