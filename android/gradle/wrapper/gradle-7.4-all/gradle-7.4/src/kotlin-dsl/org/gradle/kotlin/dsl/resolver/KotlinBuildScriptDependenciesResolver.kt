...
class KotlinBuildScriptDependenciesResolver @VisibleForTesting constructor(
    ...
    suspend fun assembleDependenciesWithShortCircuit(
        ...
        when (val action =
            ResolverCoordinator.selectNextActionFor(script, environment, previousDependencies)) {
...
object ResolverCoordinator {
    ...
    private
    fun sameClassPathBlocksHashAs(
        previousDependencies: KotlinScriptExternalDependencies?,
        hash: ByteArray?
    ) =
        hash?.let { nonNullHash ->
            classPathBlocksHashOf(previousDependencies)?.contentEquals(
                nonNullHash
            )
        }
            ?: false
    ...
    fun classPathBlocksHashFor(script: ScriptContents, environment: Environment): ByteArray? {
        ...
        val getScriptSectionTokens =
            environment["getScriptSectionTokens"] as? ScriptSectionTokensProvider
