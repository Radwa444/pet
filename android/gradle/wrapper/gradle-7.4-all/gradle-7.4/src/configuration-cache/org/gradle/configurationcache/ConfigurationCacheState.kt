...
class ConfigurationCacheState(
    ...
    internal
    suspend fun DefaultWriteContext.writeBuildState(
        build: VintageGradleBuild,
    ...
    private
    suspend fun DefaultWriteContext.writeWorkGraphOf(
    ...
    private
    suspend fun DefaultWriteContext.writeRequiredBuildServicesOf(
        gradle: GradleInternal,
        buildTreeState: StoredBuildTreeState
    ) {
    ...
    private
    suspend fun DefaultWriteContext.writeProjectStates(
        gradle: GradleInternal,
        relevantProjects: List<ProjectState>
    ) {
        withGradleIsolate(gradle, userTypesCodec) {
    ...
    private
    suspend fun DefaultWriteContext.writeRootEventListenerSubscriptions(
        gradle: GradleInternal,
        listeners: List<Provider<*>>
    ) {
        withGradleIsolate(gradle, userTypesCodec) {
    ...
    private
    suspend fun DefaultWriteContext.writeGradleState(
        gradle: GradleInternal,
        buildTreeState: StoredBuildTreeState
    ) {
        withGradleIsolate(gradle, userTypesCodec) {
    ...
    private
    suspend fun DefaultWriteContext.writeChildBuilds(
        gradle: GradleInternal,
        buildTreeState: StoredBuildTreeState
    ) {
        ...
        val cachedBuildState =
            if (stored) {
                val confCacheBuild = includedBuild.withState { includedGradle ->
                    includedGradle.serviceOf<ConfigurationCacheHost>()
                        .createBuild(null, includedBuild.name)
                }
        ...
        readCollection {
            when (readBoolean()) {
                true -> {
                    ...
                    val provider = buildStateRegistry.buildServiceRegistrationOf(buildIdentifier)
    ...
    private
    fun fireConfigureBuild(
        buildOperationExecutor: BuildOperationExecutor,
        gradle: GradleInternal,
        function: (gradle: GradleInternal) -> Unit
    ) {
        BuildOperationFiringProjectsPreparer(function, buildOperationExecutor).prepareProjects(
            gradle
        )
    }
