...
class ConfigurationCacheProblems(
    ...
    override fun report(reportDir: File, validationFailures: Consumer<in Throwable>) {
        ...
        val htmlReportFile = report.writeReportFileTo(
            outputDirectory,
            cacheActionText,
            requestedTasks,
    ...
    inner class PostBuildProblemsHandler : RootBuildLifecycleListener {
        ...
        override fun beforeComplete() {
            ...
            val discardStateDueToProblems =
                (problemCount > 0 || hasIncompatibleTypes) && isFailOnProblems
            ...
            when {
                ...
                isFailingBuildDueToSerializationError -> log(
                    "Configuration cache entry discarded with {}.",
                    problemCountString
                )

                cacheAction == STORE && discardStateDueToProblems && !hasProblems -> log("Configuration cache entry discarded.")
                cacheAction == STORE && discardStateDueToProblems -> log(
                    "Configuration cache entry discarded with {}.",
                    problemCountString
                )

                cacheAction == STORE && hasTooManyProblems -> log(
                    "Configuration cache entry discarded with too many problems ({}).",
                    problemCountString
                )

                cacheAction == STORE && !hasProblems -> log("Configuration cache entry stored.")
                cacheAction == STORE -> log(
                    "Configuration cache entry stored with {}.",
                    problemCountString
                )

                cacheAction == UPDATE && !hasProblems -> log(
                    "Configuration cache entry updated for {}, {} up-to-date.",
                    updatedProjectsString,
                    reusedProjectsString
                )

                cacheAction == UPDATE -> log(
                    "Configuration cache entry updated for {} with {}, {} up-to-date.",
                    updatedProjectsString,
                    problemCountString,
                    reusedProjectsString
                )
                ...
                cacheAction == LOAD -> log(
                    "Configuration cache entry reused with {}.",
                    problemCountString
                )

                hasTooManyProblems -> log(
                    "Too many configuration cache problems found ({}).",
                    problemCountString
                )
