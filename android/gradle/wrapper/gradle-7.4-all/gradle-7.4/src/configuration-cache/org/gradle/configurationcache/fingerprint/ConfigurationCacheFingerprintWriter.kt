...
class ConfigurationCacheFingerprintWriter(
    ...
    private
    val buildScopedWriter =
        ScopedFingerprintWriter<ConfigurationCacheFingerprint>(buildScopedContext)

    private
    val buildScopedSink = BuildScopedSink(host, buildScopedWriter)

    private
    val projectScopedWriter =
        ScopedFingerprintWriter<ProjectSpecificFingerprint>(projectScopedContext)
    ...
    private
    var closestChangingValue: ConfigurationCacheFingerprint.ChangingDependencyResolutionValue? =
        null
    ...
    override fun onDynamicVersionSelection(
        ...
        onChangingValue(
            ConfigurationCacheFingerprint.DynamicDependencyVersion(
    ...
    override fun onChangingModuleResolve(moduleId: ModuleComponentIdentifier, expiry: Expiry) {
        val expireAt = host.buildStartTime + expiry.keepFor.toMillis()
        onChangingValue(
            ConfigurationCacheFingerprint.ChangingModule(
    ...
    override fun systemPropertiesPrefixedBy(prefix: String, snapshot: Map<String, String?>) {
        buildScopedSink.write(
            ConfigurationCacheFingerprint.SystemPropertiesPrefixedBy(
                prefix,
                snapshot
            )
        )
    ...
    override fun envVariablesPrefixedBy(prefix: String, snapshot: Map<String, String?>) {
        buildScopedSink.write(
            ConfigurationCacheFingerprint.EnvironmentVariablesPrefixedBy(
                prefix,
                snapshot
            )
        )
    ...
    fun <T> collectFingerprintForProject(identityPath: Path, action: () -> T): T {
        ...
        val projectSink = sinksForProject.computeIfAbsent(identityPath) {
            ProjectScopedSink(
                host,
                identityPath,
                projectScopedWriter
    ...
    override fun dependencyObserved(
        consumingProject: ProjectState?,
        targetProject: ProjectState,
        requestedState: ConfigurationInternal.InternalState,
        target: ResolvedProjectConfiguration
    ) {
        if (host.cacheIntermediateModels && consumingProject != null) {
            val dependency = ProjectSpecificFingerprint.ProjectDependency(
                consumingProject.identityPath,
                targetProject.identityPath
            )
    ...
    override fun onProjectReference(referrer: ProjectState, target: ProjectState) {
        if (host.cacheIntermediateModels) {
            val dependency = ProjectSpecificFingerprint.CoupledProjects(
                referrer.identityPath,
                target.identityPath
            )
            if (projectDependencies.add(dependency)) {
                projectScopedWriter.write(dependency)
