...
class JavaObjectSerializationCodec(
                ...
                Format.WriteObject -> {
                    withImmediateMode {
                        decodingBeanWithId(id) { bean, beanType, beanStateReader ->
                            ...
                            when {
                                readObject.isNotEmpty() -> invokeAll(
                                    readObject,
                                    bean,
                                    objectInputStream
                                )
                ...
                Format.Broken -> {
                    decodingBeanWithId(id) { bean, _, _ ->
                        ...
                        logPropertyProblem(
                            "deserialize",
                            brokenValue.failure,
                            NotYetImplementedJavaSerialization
                        ) {
                            failedJOS(bean)
                        }
                    }
    ...
    private
    fun ReadContext.objectInputStreamAdapterFor(
        bean: Any,
        beanStateReader: BeanStateReader
...
internal
fun Iterable<Method>.serializationMethodHierarchy(
    methodName: String,
    parameterType: Class<*>
): List<Method> =
    filter { method ->
...
private
inline fun ReadContext.decodingBeanWithId(
    id: Int,
    decode: (Any, Class<*>, BeanStateReader) -> Unit
): Any? {
