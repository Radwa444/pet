...
class LocalFileDependencyBackedArtifactSetCodec(
    ...
    override suspend fun WriteContext.encode(value: LocalFileDependencyBackedArtifactSet) {
        ...
        if (!noRequestedAttributes) {
            ...
            val matchingOnArtifactFormat =
                value.selector.requestedAttributes.keySet().contains(ARTIFACT_TYPE_ATTRIBUTE)
            ...
            value.artifactTypeRegistry.visitArtifactTypes { sourceAttributes ->
                val recordingSet =
                    RecordingVariantSet(value.dependencyMetadata.files, sourceAttributes)
                ...
                if (selected == ResolvedArtifactSet.EMPTY) {
                    ...
                    mappings.put(
                        sourceAttributes,
                        TransformMapping(
                            recordingSet.targetAttributes!!,
                            recordingSet.transformation!!
                        )
                    )
                } else {
    ...
    override suspend fun ReadContext.decode(): LocalFileDependencyBackedArtifactSet {
        ...
        val artifactTypeRegistry = decodePreservingSharedIdentity {
            val registry = DefaultArtifactTypeRegistry(
                instantiator,
                attributesFactory,
                CollectionCallbackActionDecorator.NOOP,
                EmptyVariantTransformRegistry
            )
        ...
        val selector = if (noRequestedAttributes) {
            ...
            FixedVariantSelector(
                matchingOnArtifactFormat,
                transforms,
                fileCollectionFactory,
                NoOpTransformedVariantFactory
            )
        }
        return LocalFileDependencyBackedArtifactSet(
...
class FixedVariantSelector(
    ...
    override fun select(
        ...
        return when (spec) {
            ...
            is TransformMapping -> factory.asTransformed(
                variant,
                spec,
                EmptyDependenciesResolverFactory(fileCollectionFactory),
...
private
class EmptyDependenciesResolverFactory(private val fileCollectionFactory: FileCollectionFactory) :
    ExtraExecutionGraphDependenciesResolverFactory, TransformUpstreamDependenciesResolver,
...
object EmptyVariantTransformRegistry : VariantTransformRegistry {
    ...
    override fun <T : TransformParameters?> registerTransform(
        ...
        registrationAction: Action<in org.gradle.api.artifacts.transform.TransformSpec<T>>
    ) {
        throw UnsupportedOperationException("Should not be called")
    }
