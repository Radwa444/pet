...
class ConfigurationCacheFingerprintChecker(private val host: Host) {
    ...
    suspend fun ReadContext.checkProjectScopedFingerprint(): CheckedFingerprint {
        ...
        return if (firstReason == null) {
            ...
            CheckedFingerprint.ProjectsInvalid(
                firstReason!!,
                projects.entries.filter { it.value.isInvalid }.map { it.key }.toSet()
            )
        }
    }

    suspend fun ReadContext.visitEntriesForProjects(
        reusedProjects: Set<Path>,
        consumer: Consumer<ProjectSpecificFingerprint>
    ) {
        while (true) {
            when (val input = read()) {
                null -> break
    ...
    private
    fun MutableMap<Path, ProjectInvalidationState>.entryFor(path: Path) =
        getOrPut(path) { ProjectInvalidationState() }
    ...
    fun check(input: ConfigurationCacheFingerprint): InvalidationReason? {
        when (input) {
            ...
            is ConfigurationCacheFingerprint.SystemPropertiesPrefixedBy -> input.run {
                val current = System.getProperties().uncheckedCast<Map<String, Any>>()
        ...
        when (val upToDatePrefix = countUpToDatePrefixOf(previous, current)) {
            ...
            current.size -> {
                ...
                when (val modifiedScript = current[upToDatePrefix]) {
                    ...
                    else -> "content of ${ordinal(upToDatePrefix + 1)} init script, '${
                        displayNameOf(
                            modifiedScript
                        )
                    }', has changed"
