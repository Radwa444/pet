...
class DefaultConfigurationCache internal constructor(
    ...
    val intermediateModels =
        lazy { IntermediateModelController(host, cacheIO, store, cacheFingerprintController) }
    ...
    override fun loadOrScheduleRequestedTasks(
        ...
        scheduler: (BuildTreeWorkGraph) -> Unit
    ) {
        if (isLoaded) {
            loadWorkGraph(graph)
        } else {
    ...
    override fun <T> loadOrCreateIntermediateModel(
        ...
        return intermediateModels.value.loadOrCreateIntermediateModel(
            identityPath,
            modelName,
            creator
        )
    ...
    override fun loadOrCreateProjectMetadata(
        identityPath: Path,
        creator: () -> LocalComponentMetadata
        ...
        if (hasSavedValues) {
            ...
            store.useForStore { layout ->
                ...
                cacheIO.writeCacheEntryDetailsTo(
                    buildStateRegistry,
                    intermediateModels.value.values,
                    projectMetadata.value.values,
                    layout.fileFor(StateType.Entry)
                )
            }
    ...
    private
    fun <T : Any> loadFromCache(
        stateType: StateType,
        action: (ConfigurationCacheStateFile) -> T
    ): T {
        prepareConfigurationTimeBarrier()
    ...
    private
    fun writeConfigurationCacheFingerprint(
        ...
        cacheFingerprintController.commitFingerprintTo(
            layout.fileFor(StateType.BuildFingerprint),
            layout.fileFor(StateType.ProjectFingerprint)
        )
    }
    ...
    fun checkFingerprintAgainstLoadedProperties(
        ...
        val projectResult =
            checkProjectScopedFingerprint(layout.fileFor(StateType.ProjectFingerprint))
        if (projectResult is CheckedFingerprint.ProjectsInvalid) {
            intermediateModels.value.restoreFromCacheEntry(
                entryDetails.intermediateModels,
                projectResult
            )
