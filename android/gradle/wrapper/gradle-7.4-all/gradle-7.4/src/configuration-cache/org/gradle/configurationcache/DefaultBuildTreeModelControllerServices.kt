...
class DefaultBuildTreeModelControllerServices : BuildTreeModelControllerServices {
    override fun servicesForBuildTree(requirements: BuildActionModelRequirements): BuildTreeModelControllerServices.Supplier {
        ...
        val parallelToolingActions =
            (isolatedProjects || requirements.startParameter.isParallelProjectExecutionEnabled) && isNotDisabled(
                requirements,
                "org.gradle.internal.tooling.parallel"
            )
        val invalidateCoupledProjects = isolatedProjects && isNotDisabled(
            requirements,
            "org.gradle.internal.invalidate-coupled-projects"
        )
    ...
    fun isNotDisabled(requirements: BuildActionModelRequirements, systemPropertyName: String) =
        !"false".equals(
            requirements.startParameter.systemPropertiesArgs.get(systemPropertyName),
            true
        )
