...
object KotlinBuildScriptModelBuilder : ToolingModelBuilder {
    ...
    fun scriptModelBuilderFor(
        ...
        return when (kotlinScriptTypeFor(scriptFile)) {
            ...
            KotlinScriptType.SETTINGS -> settingsScriptPluginModelBuilder(
                scriptFile,
                modelRequestProject
            )
    ...
    fun requestParameterOf(modelRequestProject: Project) =
        KotlinBuildScriptModelParameter(
            (modelRequestProject.findProperty(KotlinBuildScriptModel.SCRIPT_GRADLE_PROPERTY_NAME) as? String)?.let(
                ::canonicalFile
            ),
...
fun projectScriptModelBuilder(
    ...
    accessorsClassPath = { classPath ->
        ...
        val projectAccessorClassPathGenerator =
            project.serviceOf<ProjectAccessorsClassPathGenerator>()
        val dependenciesAccessors = project.serviceOf<DependenciesAccessors>()
        projectAccessorClassPathGenerator.projectAccessorsClassPath(
            project,
            classPath
        ) + pluginAccessorClassPathGenerator.pluginSpecBuildersClassPath(project) + AccessorsClassPath(
            dependenciesAccessors.classes,
            dependenciesAccessors.sources
        )
    },
...
fun compilationClassPathForScriptPluginOf(
    ...
    val scriptSource =
        textResourceScriptSource(resourceDescription, scriptFile, project.serviceOf())
...
private
fun textResourceScriptSource(
    description: String,
    scriptFile: File,
    resourceLoader: TextFileResourceLoader
) =
...
val Settings.scriptCompilationClassPath
    get() = serviceOf<KotlinScriptClassPathProvider>().safeCompilationClassPathOf(
        classLoaderScope,
        false
    ) {
