...
class DefaultPrecompiledScriptPluginsSupport : PrecompiledScriptPluginsSupport {
    ...
    companion object {
        val PRECOMPILED_SCRIPT_MANUAL =
            Documentation.userManual("custom_plugins", "sec:precompiled_plugins")!!
    }

    override fun enableOn(target: PrecompiledScriptPluginsSupport.Target): Boolean =
        target.project.run {
...
fun Project.enableScriptCompilationOf(
    ...
    val pluginSpecBuildersMetadata =
        buildDir("kotlin-dsl/precompiled-script-plugins-metadata/plugin-spec-builders")

    val compileClasspath = sourceSets["main"].compileClasspath

    tasks {

        val extractPrecompiledScriptPluginPlugins by registering(
            ExtractPrecompiledScriptPluginPlugins::class
        ) {
        ...
        if (inClassPathMode()) {
            ...
            val configurePrecompiledScriptDependenciesResolver by registering(
                ConfigurePrecompiledScriptDependenciesResolver::class
            ) {
                dependsOn(generatePrecompiledScriptPluginAccessors)
...
fun Project.validateScriptPlugin(scriptPlugin: PrecompiledScriptPlugin) {
    ...
    if (scriptPlugin.id == DefaultPluginManager.CORE_PLUGIN_NAMESPACE || scriptPlugin.id.startsWith(
            DefaultPluginManager.CORE_PLUGIN_PREFIX
        )
    ) {
        throw GradleException(
            String.format(
                ...
                DefaultPluginManager.CORE_PLUGIN_NAMESPACE,
                DefaultPluginManager.CORE_PLUGIN_NAMESPACE,
    ...
    if (existingPlugin != null && existingPlugin.javaClass.getPackage().name.startsWith(
            DefaultPluginManager.CORE_PLUGIN_PREFIX
        )
    ) {
        throw GradleException(
