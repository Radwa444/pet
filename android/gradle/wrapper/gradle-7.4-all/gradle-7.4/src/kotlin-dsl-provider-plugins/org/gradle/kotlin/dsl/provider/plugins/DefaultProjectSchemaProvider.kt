...
fun Class<*>.accessorTypePrecedenceSequence(): Sequence<Class<*>> = sequence {
    ...
    classes.forEach { `class` ->
        `class`.interfaces.forEach { `interface` ->
            when (val indexOfSupertype =
                interfaces.indexOfFirst { it.isAssignableFrom(`interface`) }) {
...
private
fun toConfigurationEntry(configuration: Configuration) =
    (configuration as DeprecatableConfiguration).run {
