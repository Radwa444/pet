...
abstract class GeneratePrecompiledScriptPluginAccessors @Inject internal constructor(
    ...
    private
    fun pluginsAppliedBy(
        scriptPlugin: PrecompiledScriptPlugin,
        scriptPluginsById: Map<String, ScriptPluginPlugins>
    ) =
    ...
    fun projectSchemaImpliedByPluginGroups(
        ...
        pluginGroupsPerRequests.flatMap { (uniquePluginRequests, scriptPlugins) ->
            try {
                val schema = projectSchemaFor(
                    pluginRequestsOf(
                        scriptPlugins.first(),
                        uniquePluginRequests
                    )
                ).get()
        ...
        return createNestedBuildTree(
            ...
            controller.withEmptyBuild { settings ->
                Try.ofFailable {
                    ...
                    val baseScope =
                        classLoaderScopeRegistry.coreAndPluginsScope.createChild("accessors-classpath")
                            .apply {
                    ...
                    val projectState = gradle.serviceOf<ProjectStateRegistry>().registerProject(
                        gradle.owner,
                        settings.rootProject as DefaultProjectDescriptor
                    )
     ...
     */
    private
    class ProjectSchemaBuildStartParameter(buildLayout: BuildLayoutParameters) :
